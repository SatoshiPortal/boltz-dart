...
fn wire_preimage_random__static_method__Api_impl(port_: MessagePort) {
    FLUTTER_RUST_BRIDGE_HANDLER.wrap::<_, _, _, PreImage, _>(
        WrapInfo {
            debug_name: "preimage_random__static_method__Api",
            port: Some(port_),
            mode: FfiCallMode::Normal,
        },
        move || move |task_callback| Result::<_, ()>::Ok(Api::preimage_random()),
    )
}
