...
mod tests {
    ...
    fn test_secrets() {
        ...
        let kps = KeyPair::new(mnemonic.clone(), Chain::BitcoinTestnet, 0, SwapType::Submarine).unwrap();
        ...
        let kps = KeyPair::new(mnemonic.clone(), Chain::BitcoinTestnet, 1, SwapType::Submarine).unwrap();
        let expected_seckey = "5416f1e024c191605502017d066786e294f841e711d3d437d13e9d27e40e066e";
        assert_eq!(&kps.secret_key, expected_seckey);
        let kps = KeyPair::new(mnemonic.clone(), Chain::BitcoinTestnet, 0, SwapType::Reverse).unwrap();
        let expected_seckey = "a0a62dd7225288f41a741c293a3220035b4c71686dc34c01ec84cbe6ab11b4e1";
        assert_eq!(&kps.secret_key, expected_seckey);
        let kps = KeyPair::new(mnemonic.clone(), Chain::BitcoinTestnet, 1, SwapType::Reverse).unwrap();
